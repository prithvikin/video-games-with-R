...
```{r}
videogames=read.csv("C:\\Users\\Prithvi Kinariwala\\Desktop\\First_Year\\2nd Semester\\STAT 1601\\Final Project\\vgsales.csv")
vgsales=videogames
library(dplyr)
library(ggplot2)
```
...
```{r}
library(ggplot2)
ggplot(data = videogames, mapping = aes(x = NA_Sales, y =Global_Sales)) +   geom_point(alpha=0.25)+
...
```{r}
library(ggplot2)
ggplot(data = videogames, mapping = aes(x = JP_Sales, y =Global_Sales)) +   geom_point(alpha=0.25)+
...
```{r}
NAGlobalModel<-lm(Global_Sales ~ NA_Sales, data = videogames)
summary(NAGlobalModel)
```

Japan/Global Sales Linearization Model
```{r}
JPGlobalModel<-lm(Global_Sales ~ JP_Sales, data = videogames)
